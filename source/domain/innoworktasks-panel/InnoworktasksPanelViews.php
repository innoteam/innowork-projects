...
class InnoworktasksPanelViews extends \Innomatic\Desktop\Panel\PanelViews
{
        ...
        $this->toolbars['mail'] = array(
            'tasks' => array(
                ...
                'action' => \Innomatic\Wui\Dispatch\WuiEventsCall::buildEventsCallString(
                    '', 
                    array(
                        array('view', 'default', array('done' => 'false'))
                    )
                )
            ),
            'donetasks' => array(
                ...
                'action' => \Innomatic\Wui\Dispatch\WuiEventsCall::buildEventsCallString(
                    '', 
                    array(
                        array('view', 'default', array('done' => 'true'))
                    )
                )
            ),
            'newtask' => array(
                ...
                array(
                    ...
                    'toolbars' => array(
                        new WuiInnomaticToolbar(
                            'view',
                            array('toolbars' => $this->toolbars, 'toolbar' => 'true')
                        ),
                        new WuiInnomaticToolBar(
                            'core',
                            array(
                                'toolbars' => $this->innoworkCore->getMainToolBar(), 
                                'toolbar' => 'true'
                            )
                        )
                    ),
    ...
    public function viewDefault($eventData)
    {
        ...
        if (isset($eventData['filter'])) {
            ...
            $type_filter_sk = new WuiSessionKey(
                'type_filter', 
                array('value' => $eventData['filter_typeid'])
            );
        ...
        $tab_sess = new WuiSessionKey(
            'innoworktaskstab', array('value' => $eventData['done'])
        );
              ...
              <children>
                ...
                <label><name>title</name>
                  <args>
                    <bold>true</bold>
                    <label>'.(
                        $this->localeCatalog->getStr(
                            (isset($eventData['done']) and $eventData['done'] == 'true') ? 'donetasks.label' : 'tasks.label'
                        )
                    ).'</label>
                  </args>
                </label>
                          ...
                          <children>
                            <link>
                              <args>
                                <label>'
                                .WuiXml::cdata(
                                    $task['id'].' - '.$country->FormatShortArrayDate(
                                ...
                                .WuiXml::cdata(
                                    \Innomatic\Wui\Dispatch\WuiEventsCall::buildEventsCallString(
                                        '', array(array('view', 'showtask', array('id' => $task['id'])))
                                    )
                                ).'</link>
                                </args>
                            </link>
                                    ...
                                    'view' => array(
                                        ...
                                        'done' => array(
                                            ...
                                            'action' => \Innomatic\Wui\Dispatch\WuiEventsCall::buildEventsCallString(
                                                ...
                                                array(
                                                    array('view', 'default', ''),
                                                    array('action', 'edittask', array('id' => $task['id'], 'done' => $done_action))
                                                )
                                            )
                                        ),
                                        'trash' => array(
                                            ...
                                            'action' => \Innomatic\Wui\Dispatch\WuiEventsCall::buildEventsCallString(
                                                ...
                                                array(
                                                    array('view', 'default', ''),
                                                    array('action', 'trashtask', array('id' => $task['id']))
                                                )
                                            )
                                        )
                                    )
    ...
    public function viewNewtask($eventData) 
    {
        // Projects
    ...
    public function viewShowtask($eventData)
    {
        ...
        $task_data = $innowork_task->getItem($this->innomaticContainer->getCurrentUser()->getUserId());
        ...
        if ($appDeps->isEnabled('innowork-userstories', $this->innomaticContainer->getCurrentDomain()->domainid)) {
        ...
        $users_query = $this->innomaticContainer->getCurrentDomain()->getDataAccess()->execute(
            'SELECT id,fname,lname '.
            'FROM domain_users '.
            'WHERE username<>'.$this->innomaticContainer->getCurrentDomain()->getDataAccess()->formatText(User::getAdminUsername($this->innomaticContainer->getCurrentDomain()->getDomainId())).' '.
            'ORDER BY lname,fname'
        );
        ...
        if ($task_data['done'] == $this->innomaticContainer->getCurrentDomain()->getDataAccess()->fmttrue) {
            $done_icon = 'misc3';
            $done_action = 'false';
            $done_label = 'setundone.button';
        } else {
                        ...
                        <args>
                            ...
                            .WuiXml::cdata(
                                \Innomatic\Wui\Dispatch\WuiEventsCall::buildEventsCallString(
                                    '',
                                    array(
                                        array('view', 'showtask', array('id' => $eventData['id'])),
                                        array('action', 'edittask', array('id' => $eventData['id']))
                                    )
                                )
                            ).'</action>
                        </args>
                            ...
                            <horizgroup>
                              ...
                              <children>
                                ...
                                <combobox><name>projectid</name>
                                  ...
                                  <events>
                                    <change>
                                        var v1 = document.getElementById(\'projectid\').value;
                                        xajax_OnChangeListUserStories(v1);
                                    </change>
                                  </events>
                                </combobox>
                              </children>
                ...
                <grid>
                  <children>
                    ...
                    <checkbox row="1" col="3" halign="left">
                        ...
                        <args>
                            <disp>action</disp><checked>'.($task_data['impediment'] == $this->innomaticContainer->getCurrentDomain()->getDataAccess()->fmttrue ? 'true' : 'false').'</checked>
                        </args>
                    </checkbox>
                  </children>
                </grid>
              ...
              <args>
                ...
                .WuiXml::cdata(
                    \Innomatic\Wui\Dispatch\WuiEventsCall::buildEventsCallString(
                        '',
                        array(
                            array('view', 'addmessage', array('taskid' => $eventData['id']))
                        )
                    )
                ).'</action>
              </args>
              ...
              <args>
                ...
                .WuiXml::cdata(
                    \Innomatic\Wui\Dispatch\WuiEventsCall::buildEventsCallString(
                        ...
                        array(
                            array('view', 'default'),
                            array('action', 'trashtask', array('id' => $eventData['id']))
                        )
                    )
                ).'</action>
              </args>
          ...
          <args>
            ...
            .WuiXml::cdata(
                \Innomatic\Wui\Dispatch\WuiEventsCall::buildEventsCallString(
                    '',
                    array(
                        array('view', 'taskmessages', array('taskid' => $eventData['id']))
                    )
                )
            ).'</source>
  ...
  <innoworkitemacl><name>itemacl</name>
    ...
    public function viewTaskmessages($eventData)
    {
        $innowork_task = new InnoworkTask(
            $this->innomaticContainer->getDataAccess(),
            $this->innomaticContainer->getCurrentDomain()->getDataAccess(),
                    ...
                    <args>
                        ...
                        .WuiXml::cdata(
                            \Innomatic\Wui\Dispatch\WuiEventsCall::buildEventsCallString(
                                '',
                                array(
                                    array('view', 'taskmessages', array('taskid' => $eventData['taskid'])),
                                    array(
                                        'action',
                                        'removemessage',
                                        array(
    ...
    {
        $domain_da = $this->innomaticContainer->getCurrentDomain()->getDataAccess();

        $query = $domain_da->execute(
            'SELECT id, name 
            FROM innowork_projects 
            WHERE name LIKE "%'.$_GET['term'].'%" 
            AND done <> '.$domain_da->formatText($domain_da->fmttrue)
        );
